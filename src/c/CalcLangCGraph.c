...
long double calcLangValueFuncWrapper(LLVMIntArena* arena, CalcLangValue* (*valFunc)(CalcLangValue*), long double val){
  CalcLangValue* cVal = arenaAlloc(arena, sizeof(CalcLangValue*));
  cVal->valType = IS_INT;
  cVal->valData.integer = (int)val;
  CalcLangValue* res = valFunc(cVal);
...
void shadeGraph(CalcLangPixel** display, CalcLangValue* (*dataFunc)(CalcLangValue*), LLVMIntArena* arena, long double xSteps, long double ySteps, long double leftBound, long double rightBound, int windowHeight, int windowWidth){
...
void drawLine(CalcLangPixel** display, CalcLangValue* (*dataFunc)(CalcLangValue*), LLVMIntArena* arena, long double xSteps, long double ySteps, int windowHeight, int windowWidth){
  long double relX, relY;
...
bool drawGraph(LLVMIntArena* arena, int begin, int end, int byAmount, CalcLangValue* (*valFunc)(CalcLangValue*)){
  struct winsize w;
